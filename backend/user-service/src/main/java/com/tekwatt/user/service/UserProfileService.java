...
public class UserProfileService {
    ...
    @Transactional public UserResponse create(CreateUserRequest r) {
        ...
        return UserResponse.from(repository.save(new UserProfile(r.authUserId(),r.tenantId(),r.firstName().trim(),r.lastName().trim(),r.email().trim().toLowerCase(),r.phone())));
    }
    ...
    @Transactional public UserResponse update(UUID id, UpdateUserRequest r) {
        ...
        u.update(r.firstName().trim(),r.lastName().trim(),email,r.phone()); return UserResponse.from(u);
    }

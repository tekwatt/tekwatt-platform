...
public class TariffClient {
    ...
    public ResolvedTariff resolve(UUID tenantId, UUID chargerId) {
        try {
            ...
            throw new ResponseStatusException(HttpStatus.CONFLICT, responseDetail(e, "Assigned tariff is not active for this time"), e);
    ...
    private String responseDetail(RestClientResponseException exception, String fallback) {
        try {
            ProblemDetail problem = exception.getResponseBodyAs(ProblemDetail.class);
            return problem != null && problem.getDetail() != null && !problem.getDetail().isBlank() ? problem.getDetail() : fallback;

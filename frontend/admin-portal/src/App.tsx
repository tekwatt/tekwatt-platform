...
import { api, Charger, ChargingSession, Connector, FirmwareJob, FirmwarePackage, Notification, OcppConnection, OcppMessage, Payment, Report, SupportTicket, Telemetry, Tenant, TicketDetail, UserProfile } from './api';

type Page = 'Dashboard' | 'Stations' | 'Sessions' | 'Payments' | 'Users' | 'Reports' | 'Support' | 'Settings';
type LiveData = { tenant?: Tenant; chargers: Charger[]; connectors: Connector[]; sessions: ChargingSession[]; payments: Payment[]; users: UserProfile[]; reports: Report[]; notifications: Notification[]; connections: OcppConnection[]; messages: OcppMessage[]; firmwarePackages: FirmwarePackage[]; firmwareJobs: FirmwareJob[]; supportTickets: SupportTicket[] };
...
function Portal({ logout, demo }: { logout: () => void; demo: boolean }) {
  ...
  const [data, setData] = useState<LiveData>({ chargers: [], connectors: [], sessions: [], payments: [], users: [], reports: [], notifications: [], connections: [], messages: [], firmwarePackages: [], firmwareJobs: [], supportTickets: [] });
  ...
  const refresh = async (silent = false) => {
    ...
    try {
      ...
      const results = await Promise.allSettled([
        api.chargers(tenant.id), api.sessions(tenant.id), api.payments(tenant.id), api.users(tenant.id), api.reports(tenant.id), api.notifications(tenant.id), api.ocppConnections(), api.ocppMessages(), api.firmwarePackages(), api.firmwareJobs(tenant.id), api.supportTickets(tenant.id),
      ]);

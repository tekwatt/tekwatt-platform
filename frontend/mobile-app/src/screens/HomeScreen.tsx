...
export function HomeScreen() {
  ...
  return <Screen refreshing={loading} onRefresh={load}>
    ...
    {active.length ? active.map(session => <Card key={session.id}><View style={commonStyles.between}><View style={styles.flex}><Text style={commonStyles.strong}>{chargers.find(item => item.id === session.chargerId)?.stationName || chargers.find(item => item.id === session.chargerId)?.stationId || 'Charging station'}</Text><Text style={commonStyles.tiny}>{session.transactionId}</Text></View><Pill label={session.status} tone="green"/></View><View style={commonStyles.divider}/><View style={styles.liveValues}><View><Text style={styles.energy}>{formatChargingDuration(chargingDurationMs(session,now))}</Text><Text style={commonStyles.tiny}>CHARGING TIME</Text></View><View><Text style={styles.energy}>{Number(session.energyKwh || 0).toFixed(2)} kWh</Text><Text style={commonStyles.tiny}>ENERGY DELIVERED</Text></View></View><Text style={commonStyles.body}>Session in progress. Open Charging for cost, charger power and controls.</Text></Card>) : <EmptyState title="No active session" message="Choose an available connector from Stations when you are ready to charge."/>}
...
const styles = StyleSheet.create({ metrics: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', rowGap: 12 }, flex: { flex: 1 }, liveValues:{flexDirection:'row',justifyContent:'space-between',gap:16}, energy: { color: colors.blue, fontSize: 22, fontWeight: '900', marginBottom: 5 } });
